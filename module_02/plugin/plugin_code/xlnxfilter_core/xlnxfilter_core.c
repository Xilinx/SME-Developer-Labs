...
int xlnxfilter_core(const AVFrame *pic, const AVFrame *dst, unsigned int w, unsigned int h, unsigned int coeff_select, int ncompute_unit, unsigned int both)
{
	...
	oclRequest* request[3];
      ...
      for(int xx=0; xx<numRuns; xx++) {
            ...
            request[xx*3+0] = Execute(Filter,pic->data[0], width, height, stride, coeff_array, dst->data[0]);
            request[xx*3+1] = Execute(Filter,pic->data[1], width, height, stride, coeff_array, dst->data[1]);
            request[xx*3+2] = Execute(Filter,pic->data[2], width, height, stride, coeff_array, dst->data[2]);
 ...
 if((ncompute_unit == 0) || (both==1)) {
      printf("\nRunning Software version\n");
        ...
        uint8_t *u_ref = (uint8_t*) malloc(sizeof(uint8_t)*height*width);
        uint8_t *v_ref = (uint8_t*) malloc(sizeof(uint8_t)*height*width);
      clock_t begin = clock();
      for(int xx=0; xx<numRuns; xx++) {
